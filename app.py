...
def login():
    ...
    if request.method == 'POST':
        ...
        # ── حماية Brute-Force: تأخير تصاعدي ──────────────────────────
        lock_row = query(
            "SELECT failed_count, locked_until FROM login_lockouts WHERE email=? AND ip=?",
            (email, ip), one=True
        )
        if lock_row and lock_row.get('locked_until'):
            ...
            try:
                ...
                if locked_until > now:
                    ...
                    error = f'الحساب مقفل مؤقتاً. انتظر {wait_mins} دقيقة قبل المحاولة مجدداً.'
        ...
        if user:
            ...
            if is_valid:
                ...
                execute("DELETE FROM login_lockouts WHERE email=? AND ip=?", (email, ip))
                ...
                if lock_row:
                    execute(
                        "UPDATE login_lockouts SET failed_count=?, locked_until=?, last_attempt=? WHERE email=? AND ip=?",
                        (new_count, locked_until_str, now_str, email, ip)
                    )
                else:
                    execute(
                        "INSERT INTO login_lockouts(email, ip, failed_count, locked_until, last_attempt) VALUES(?,?,?,?,?)",
                        (email, ip, new_count, locked_until_str, now_str)
                    )

                remaining = 5 - new_count
                if lock_mins:
                    error = f'كلمة المرور خاطئة. تم قفل الحساب لمدة {lock_mins} دقيقة.'
                elif remaining > 0:
...
def class_analytics(class_id):
    ...
    for stu in students:
        ...
        if general_avg is None:
            ...
            subject_grades = {}
            for code, _ in SUBJECTS:
                ag = calc_subject_annual(stu['id'], code, yr, class_id)
                if ag is not None:
...
def archives_purge(entity, eid):
    try:
        if entity == 'class':
            ...
            execute('DELETE FROM grades     WHERE student_id=?', (eid,))
            execute('DELETE FROM attendance WHERE student_id=?', (eid,))
